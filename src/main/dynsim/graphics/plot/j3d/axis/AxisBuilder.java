...
public class AxisBuilder {
	...
	AxisBuilder() {
		...
		label.setCapability(Text3D.ALLOW_FONT3D_WRITE);
		label.setCapability(Text3D.ALLOW_STRING_WRITE);
		...
		ticks.setCapability(Group.ALLOW_CHILDREN_READ);
		ticks.setCapability(Group.ALLOW_CHILDREN_WRITE);
		ticks.setCapability(BranchGroup.ALLOW_DETACH);
	...
	/**
	 * Tick labels and locations (positions) can be set by the caller or
	 * calculated and set by the createLabelsNTicks method as a convenience.
	 */
	...
	/**
	 * Call the createLabelsNTicks method if you would like the axisbuilder to
	 * create axis labels and tick positions for you.
	 */
	...
	public void apply() {
		 ...
		 * 256 is to compensate for the giant size of the Text3D characters. The
		 * axis is built in the XY plane. We will use suitable translation and
		 * rotation to position it later.
		 * 
		 * Note we currently use Text3D to draw the labels. This is because
		 * Text3D seems somewhat easier to use. A better solution would probably
		 * involve using Text2D, but the com.sun.j3d.utils.geometry.Text2D class
		 * seems pretty brain dead. A better solution may be to get the source
		...
		// we would keep as many as of the old labels as possible,
		// adding/removing
		// new tick marks only as the number of tick marks change, and changing
		// the
		...
		for (int i = 0; i < tickLocations.length; i++) {
			...
			if (i % tMajor == 0) // Major tick mark?
			{
				...
				tickLabel.setAlignment(Text3D.ALIGN_CENTER);
		...
		// TODO: It would be more efficient to only update the label if
	...
	/**
	 * Returns the node representing this Axis Subclasses can override this
	 * method to transform this axis to make it into an X,Y,Z axis.
	 */
	public Node getNode() {
		return mainGroup;
	}
